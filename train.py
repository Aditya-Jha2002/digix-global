...
def get_input_data(train_df, test_df, config):
    ...
    all_data_df = pd.concat([train_df.head(20), test_df.head(20)])
    feature_columns = []
    dim_dict = {}
    ...
    for fc in feature_columns:
        if isinstance(fc, SparseFeat):
            ...
            train_input[fc.name] = np.array(gen_seq_data(train_df, fc, config)).astype(int).tolist()
            test_input[fc.name] = np.array(gen_seq_data(test_df, fc, config)).astype(int).tolist()
    train_input['label'] = train_df['label'].values.astype(int)
    test_input['log_id'] = test_df['log_id'].values.astype(int)

    

    with open(os.path.join(output_dir, Const.INTERIM_TRAIN_DATA_FILE), 'w') as fp:
        json.dump(train_input, fp)
        
    with open(os.path.join(output_dir, Const.INTERIM_TRAIN_DATA_FILE), 'w') as fp:
        json.dump(test_input, fp)
    
    # open file in write mode
    with open(os.path.join(output_dir, 'feature_list.txt'), 'w') as fp:
        for fc in feature_columns:
